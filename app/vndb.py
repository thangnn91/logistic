...
class DBHelper(object):
    ...
    def get_rowid(self):
        if self._current_cursor:
            return int(self._current_cursor.lastrowid)
        return False
    ...
    def execute_sp(self, sql, params=None):
        if not self._tuple_cursor:
            self._tuple_cursor = self.get_cursor()
        else:
            self._conn.ping()
        try:
            self._tuple_cursor.callproc(sql, self.format_param(params))
            self._current_cursor = self._tuple_cursor
            if self._autocommit:
                self._conn.commit()
        except Exception as e:
    ...
    def insert_batch(self, sql, params, step_size=2000):
        ...
        try:
            ...
            self._conn.rollback()
            raise e
        return self.get_rowid()
    
    def update_batch(self, sql, params, step_size=2000):
        if not params:
            raise Exception("No params")
        if not self._current_cursor:
            self._tuple_cursor = self.get_cursor()
    ...
    def execute_multiple_batch(self, sql_object, step_size=2000):
        if not self._current_cursor:
            self._tuple_cursor = self.get_cursor()
            self._current_cursor = self._tuple_cursor
        else:
        ...
        row_count = 0    
        try:
            for i in range(sql_object.length):
                if sql_object.item[i].is_many == True:
                    size = int(step_size)
                    length = len(sql_object.item[i].param)
                    size = size if length >= size else length
                    params = self.format_param(sql_object.item[i].param, True)
                    
                    for j in xrg(0, (length + size - 1) // size):
                        self._current_cursor.executemany(sql_object.item[i].query, params[j * size:(j + 1) * size])
                        row_count = row_count + self.get_rowcount()
                else:
                    self._current_cursor.execute(sql_object.item[i].query, self.format_param(sql_object.item[i].param))
                    row_count = row_count + self.get_rowcount()
        except Exception as e:
            self._conn.rollback()
            raise e
